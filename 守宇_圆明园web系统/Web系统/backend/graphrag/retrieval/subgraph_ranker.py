...
def rank_and_assess_events(
    ...
    # ── 纯 Embedding 语义排序（去掉 n-gram 噪音）──
    emb = getattr(store, "emb_index", None)
    if emb is not None and emb:
        # 用子问题文本做语义检索
        sem_results = emb.search_events(subq.text, top_k=50, min_score=0.3,
                                        event_ids=set(candidate_scores.keys()))
        for event_id, sem_score in sem_results:
            candidate_scores[event_id] = candidate_scores.get(event_id, 0) + sem_score * 5.0
    else:
        # 回退：无 embedding 时用 n-gram
