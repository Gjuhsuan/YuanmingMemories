...
class GraphSubstrate:
    ...
    emb_index: object = None  # EmbeddingIndex，构建时自动填充
...
def build_graph_substrate(
    ...
    # ── 构建 Embedding 索引 ──
    _build_embedding_index(store)

    return store


def _build_embedding_index(store: GraphSubstrate):
    """为所有事件、实体、文档构建语义 embedding 索引。"""
    try:
        from ..retrieval.embedding_index import EmbeddingIndex

        store.emb_index = EmbeddingIndex()

        # 事件：名称 + 类型 + 日期 + 原文摘要
        for event_id, event in store.events.items():
            source = str(event.get('source_text', ''))[:200] if event.get('source_text') else ''
            text = f"{event.get('name', '')}。类型：{event.get('event_type', '')}。日期：{event.get('date_text', '')}。{source}"
            store.emb_index.add(event_id, text, {"type": "event", "name": str(event.get("name", ""))})

        # 实体：名称 + 类型 + 别名
        for entity_id, entity in store.entities.items():
            aliases_raw = entity.get("aliases", [])
            if isinstance(aliases_raw, (list, set)):
                aliases = "、".join(list(aliases_raw)[:5])
            else:
                aliases = str(aliases_raw)
            text = f"{entity.get('name', '')}。类型：{entity.get('entity_type', '')}。{aliases}"
            store.emb_index.add(entity_id, text, {"type": "entity", "name": str(entity.get("name", ""))})

        # 文档：标题 + 日期 + 正文摘要
        for doc_id, entry in store.entries.items():
            body = str(entry.get('body', ''))[:300] if entry.get('body') else ''
            text = f"{entry.get('title', '')}。日期：{entry.get('date', '')}。{body}"
            store.emb_index.add(f"doc_{doc_id}", text, {"type": "document", "name": str(entry.get("title", ""))})

        store.emb_index.build(batch_size=64)
    except ImportError:
        import logging
        logging.getLogger(__name__).warning("[embedding] sentence-transformers 未安装，跳过语义索引")
        store.emb_index = None
    except Exception as e:
        import logging
        logging.getLogger(__name__).warning(f"[embedding] 索引构建失败: {e}")
        store.emb_index = None
