...
10. CRITICAL — every subquestion MUST be self-contained and embedding-searchable. NEVER use pronouns, demonstratives, or cross-references (e.g. "these", "those", "上述", "前述", "该", "这些", "其"). Instead, repeat ALL explicit entity names, time periods, and place names from the original question in EVERY subquestion. Each subquestion must work as a standalone dense retrieval query when issued independently.
11. Each subquestion's seed_queries MUST include the concrete surface forms of all hard constraints (entities, dates, places) identified from the original question. Do not use abstract or generic seed queries.
12. Phrase all subquestions in Chinese matching the question language.
"""
